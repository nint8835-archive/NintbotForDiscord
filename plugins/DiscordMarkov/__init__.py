...
class Plugin(BasePlugin):
    ...
    async def on_command(self, args):
        if args["command_args"][0] == "wisdom" and len(args["command_args"]) == 1:
        ...
        elif args["command_args"][0] == "cleanstrings" and self.cleanstrings_permission_group.has_permission(args["author"]):
            count = 0
        ...
        if args["command_args"][0] == "wisdom" and len(args["command_args"]) >= 2:
            await self.bot.send_typing(args["channel"])
            try:
                message_string = ""
                count = 0
        ...
        if args["command_args"][0] == "web_learn" and len(args["command_args"])==2 and Owner(self.bot).has_permission(args["member"]):
            data = requests.get(args["command_args"][1]).text
            self.strings.append(data)
            self.save_strings()

        if args["command_args"][0] == "messages":
